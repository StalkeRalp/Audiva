...
export function LecteurPersistant() {
  ...
  return <><audio ref={audioRef} src={track.streamUrl} preload="metadata" onTimeUpdate={(event) => setCurrentTime(event.currentTarget.currentTime)} onLoadedMetadata={(event) => setDuration(event.currentTarget.duration)} onEnded={handleTrackEnd} />
    {lyricsOpen && <aside className="fixed bottom-[76px] right-3 z-50 w-[min(390px,calc(100vw-24px))] animate-in slide-in-from-bottom-3 rounded-2xl border border-[#75eee8]/20 bg-[#0a1328]/95 p-5 shadow-2xl backdrop-blur-xl"><div className="flex items-center justify-between"><div><p className="text-xs font-bold uppercase tracking-[.16em] text-[#75eee8]">Paroles</p><p className="mt-1 text-sm font-semibold">{track.title} · {track.artist}</p></div><button onClick={toggleLyrics} className={controlClass} aria-label="Fermer les paroles"><HugeiconsIcon icon={Cancel01Icon} size={20} /></button></div>{track.lyrics ? <div className="mt-5 max-h-64 space-y-3 overflow-y-auto pr-2">{track.lyrics.map((line) => <button type="button" key={line.time} onClick={() => { audioRef.current.currentTime = line.time; seek(line.time); }} className={`block w-full text-left text-base font-bold leading-6 transition ${line.time === activeLyric ? "scale-[1.02] text-[#75eee8]" : "text-[#73819f] hover:text-white"}`}>{line.text}</button>)}</div> : <p className="mt-6 rounded-xl bg-white/5 p-4 text-sm text-[#aeb9d0]">Paroles indisponibles pour cette chanson.</p>}</aside>}
    {queueOpen && <aside role="dialog" aria-label="File d&apos;attente" className="fixed bottom-[72px] right-0 top-0 z-50 flex w-[min(420px,100vw)] flex-col border-l border-white/10 bg-[#0a0a0b] shadow-[-16px_0_45px_rgba(0,0,0,.38)] animate-in slide-in-from-right-5 duration-200">
      ...
      {queueTab === "queue" ? <QueueList queue={queue} currentIndex={currentIndex} currentTrack={track} onPlay={(item) => { setCurrentTrack(item); play(); }} onRemove={removeFromQueue} onMove={moveQueueItem} onPlayNext={playNext} onLike={toggleLike} onLyrics={(item) => { setCurrentTrack(item); if (!lyricsOpen) toggleLyrics(); }} onNavigate={(query) => router.push(`/recherche?query=${encodeURIComponent(query)}`)} onNotice={notifyQueue} /> : <RecentList queue={queue} currentIndex={currentIndex} onPlay={(item) => { setCurrentTrack(item); play(); }} />}
...
function QueueList({ queue, currentIndex, currentTrack, onPlay, onRemove, onMove, onPlayNext, onLike, onLyrics, onNavigate, onNotice }) {
  ...
  return (
    <div className="min-h-0 flex-1 overflow-y-auto px-3 pb-5 pt-5">
      ...
      <div className="mt-6 flex items-center justify-between px-2"><p className="text-sm font-extrabold text-white">À suivre dans : votre file Audiva</p><span className="text-[10px] font-semibold text-[#8794ad]">Glissez pour réorganiser</span></div>
...
function RecentList({ queue, currentIndex, onPlay }) {
  const listened = queue.slice(0, currentIndex).reverse();
  return (
    <div className="min-h-0 flex-1 overflow-y-auto px-3 pb-5 pt-5">
