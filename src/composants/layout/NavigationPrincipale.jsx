...
import { Add01Icon, CompassIcon, FavouriteIcon, Home01Icon, LibraryIcon, Message01Icon, Notification03Icon, Search01Icon, Settings01Icon, UserGroupIcon } from "@hugeicons/core-free-icons";
...
const sections = [
  ...
  { label: "Ma musique", items: [{ href: "/bibliotheque", label: "Bibliothèque", icon: LibraryIcon }, { href: "/favoris", label: "Favoris", icon: FavouriteIcon }] },
  ...
  { label: "Compte", items: [{ href: "/bibliotheque?import=1", label: "Importer ma musique", icon: Add01Icon, action: true }, { href: "/parametres", label: "Paramètres", icon: Settings01Icon }] },
];
      ...
      <nav className="space-y-5">
        {sections.map((section) => <div key={section.label}><p className="mb-1 px-3 text-[10px] font-semibold uppercase tracking-[.16em] text-[#7180aa]">{section.label}</p>{section.items.map((item) => {
          ...
          <Link
            key={item.href}
            href={item.href}
            className={`group relative mb-0.5 flex items-center gap-3 rounded-lg px-3 py-2.5 text-sm transition-all duration-200 ${active ? "bg-[#17254a] font-medium text-white shadow-[inset_2px_0_0_#72eee7]" : item.action ? "border border-dashed border-[#72eee7]/35 bg-[#72eee7]/[.055] font-semibold text-[#c9fffb] hover:border-[#72eee7]/70 hover:bg-[#72eee7]/10" : "text-[#aab5d0] hover:bg-white/[.055] hover:text-white"}`}
          >
            <HugeiconsIcon icon={item.icon} size={18} strokeWidth={1.8} className={`transition-transform duration-200 group-hover:scale-110 ${active || item.action ? "text-[#72eee7]" : "text-[#8e9bbd]"}`} />
