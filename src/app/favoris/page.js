...
export default function FavorisPage() {
  ...
  const { likedTrackIds, likedTracks, currentTrack, isPlaying, setCurrentTrack, setQueue, play, pause, toggleLike, addToQueue } = useLecteurStore(); const { tracks: localTracks } = useBibliothequeStore();
  const favoritePlaylists = usePlaylistsStore((state) => state.favoritePlaylists);
  const togglePlaylistFavorite = usePlaylistsStore((state) => state.togglePlaylistFavorite);

  const favorites = useMemo(() => {
    const collection = [...new Map([...demoQueue, ...localTracks, ...likedTracks].map((track) => [track.id, track])).values()].filter((track) => likedTrackIds.includes(track.id));
  ...
  }, [likedTrackIds, likedTracks, localTracks, query, sort]);
  ...
  return <main className="min-h-screen bg-[#060b18] pb-32 text-[#eff4ff] lg:flex">
    <div className="min-w-0 flex-1">
      ...
      <section className="mx-auto w-full max-w-[1500px] px-4 py-8 sm:px-7 xl:px-9">
        <section className="mb-10"><div className="flex items-end justify-between gap-4"><div><p className="text-[11px] font-bold uppercase tracking-[.15em] text-[#83eee8]">Playlists</p><h2 className="mt-2 text-2xl font-black text-white">Playlists favorites</h2><p className="mt-1 text-sm text-[#91a0bd]">Vos sélections enregistrées, séparées de vos morceaux.</p></div><span className="text-xs font-bold text-[#91a0bd]">{favoritePlaylists.length} playlist{favoritePlaylists.length !== 1 ? "s" : ""}</span></div>{favoritePlaylists.length ? <div className="mt-5 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">{favoritePlaylists.map((playlist) => <article key={playlist.id} className="group overflow-hidden border border-white/[.08] bg-[#0a1221] p-3 transition hover:-translate-y-1 hover:border-[#72eee7]/35"><Link href={`/playlists/${playlist.id}`} className="relative block aspect-square overflow-hidden"><Image src={playlist.cover || "/hero-playlist.jpg"} alt="" fill sizes="260px" className="object-cover transition duration-500 group-hover:scale-105" /></Link><div className="mt-3 flex items-start gap-2"><Link href={`/playlists/${playlist.id}`} className="min-w-0 flex-1"><p className="truncate text-sm font-extrabold text-white">{playlist.title}</p><p className="mt-1 truncate text-xs text-[#91a0bd]">{playlist.tracks?.length || 0} morceau{(playlist.tracks?.length || 0) > 1 ? "x" : ""}</p></Link><button type="button" onClick={() => togglePlaylistFavorite(playlist)} aria-label="Retirer des favoris" className="grid h-9 w-9 place-items-center text-[#72eee7] transition hover:bg-white/10"><HugeiconsIcon icon={FavouriteIcon} size={19} fill="currentColor" /></button></div></article>)}</div> : <div className="mt-5 border border-dashed border-white/15 bg-[#0a1221] px-5 py-8 text-sm text-[#91a0bd]">Ajoutez une playlist aux favoris depuis sa page de détail.</div>}</section>
        <div className="mb-5"><p className="text-[11px] font-bold uppercase tracking-[.15em] text-[#83eee8]">Morceaux</p><h2 className="mt-2 text-2xl font-black text-white">Morceaux favoris</h2></div>
